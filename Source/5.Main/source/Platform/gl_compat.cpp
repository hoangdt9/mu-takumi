...
void GL_Compat_Init() {
    ...
    s_batchVerts.reserve(131072); // pre-alloc for ~3.6 frames worth at peak 100K verts/frame
    s_hasBatch = false;
    s_batchPrimMode = 0;
    s_vboCapacity = 0;
...
static void BatchAppendTrianglesFromFloatArrays(const uint8_t* posBase,
                                                ...
                                                int colorSize,
                                                int count) {
    if (!posBase || !texBase || count <= 0 || !s_prog) return;
...
void GL_DrawArrays_Compat(GLenum mode, int first, int count) {
        ...
        s_vaTexCoord.type == GL_FLOAT && s_vaTexCoord.size >= 2)
    {
        ...
        const bool tightlyPackedTex = (texStride == 2 * (int)sizeof(float));
        ...
        if (tightlyPackedPos && tightlyPackedTex)
        {
            ...
            const float* texPtr = (const float*)((const uint8_t*)s_vaTexCoord.ptr + (size_t)first * texStride);
...
void GL_BatchAppendTriangles(const float* positions,
                             ...
                             int numVerts)
{
    if (!positions || !texcoords || numVerts <= 0 || !s_prog) return;
