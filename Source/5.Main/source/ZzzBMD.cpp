	...
	const float constantColor[4])
{
	...
	buffers.positions.reserve(static_cast<size_t>(numVertices) * 3);
	buffers.colors.reserve(static_cast<size_t>(numVertices) * 4);
	buffers.texcoords.reserve(static_cast<size_t>(numVertices) * 2);
	...
	for (int triangleIndex = 0; triangleIndex < mesh.NumTriangles; ++triangleIndex)
	{
		...
		for (int corner = 0; corner < 3; ++corner)
		{
			...
			buffers.positions.push_back(vertex[0]);
			buffers.positions.push_back(vertex[1]);
			buffers.positions.push_back(vertex[2]);
			...
			case RENDER_TEXTURE:
				{
					...
					buffers.texcoords.push_back(enableWave ? (texCoord->TexCoordU + blendMeshTexCoordU) : texCoord->TexCoordU);
					buffers.texcoords.push_back(enableWave ? (texCoord->TexCoordV + blendMeshTexCoordV) : texCoord->TexCoordV);

					if (enableLight)
					{
						const int normalIndex = triangle->NormalIndex[corner];
						const float* light = LightTransform[meshIndex][normalIndex];
						AppendMobileBatchColor(buffers.colors, light[0], light[1], light[2], vertexAlpha);
					}
					else
					{
						AppendMobileBatchColor(buffers.colors, constantColor[0], constantColor[1], constantColor[2], constantColor[3]);
					}
				}
				break;
			case RENDER_CHROME:
				{
					const int normalIndex = triangle->NormalIndex[corner];
					buffers.texcoords.push_back(g_chrome[normalIndex][0]);
					buffers.texcoords.push_back(g_chrome[normalIndex][1]);
					AppendMobileBatchColor(buffers.colors, constantColor[0], constantColor[1], constantColor[2], constantColor[3]);
				}
				break;
			case RENDER_CHROME4:
				{
					const int normalIndex = triangle->NormalIndex[corner];
					buffers.texcoords.push_back(g_chrome[normalIndex][0] + blendMeshTexCoordU);
					buffers.texcoords.push_back(g_chrome[normalIndex][1] + blendMeshTexCoordV);
					AppendMobileBatchColor(buffers.colors, constantColor[0], constantColor[1], constantColor[2], constantColor[3]);
				}
				break;
			case RENDER_OIL:
				{
					...
					buffers.texcoords.push_back(g_chrome[oilIndex][0] * texCoord->TexCoordU + blendMeshTexCoordU);
					buffers.texcoords.push_back(g_chrome[oilIndex][1] * texCoord->TexCoordV + blendMeshTexCoordV);
					AppendMobileBatchColor(buffers.colors, constantColor[0], constantColor[1], constantColor[2], constantColor[3]);
				}
				break;
			default:
