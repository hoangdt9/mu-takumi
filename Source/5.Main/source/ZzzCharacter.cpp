...
namespace
{
    ...
    {
#if defined(__ANDROID__) || defined(MU_IOS)
        if (!IsCharacterAdaptiveEnabled() || c == nullptr || isPriority || !IsAdaptiveMonsterCharacter(c))
        {
            return false;
        }
...
#if defined(__ANDROID__) || defined(MU_IOS)
        if (!IsCharacterAdaptiveEnabled() || c == nullptr)
        {
            return;
        }
        ...
        const bool isPriority)
    {
        ...
        g_characterRenderLodContext.reuseAnimationPose =
...
void MoveCharactersClient()
{
    ...
    const uint32_t moveBudgetPeriod = adaptiveEnabled
