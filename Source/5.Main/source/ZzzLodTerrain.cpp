...
struct TerrainBatchQuad
{
    uint16_t textureId;
    uint8_t blendState;
    uint8_t primType;
    TerrainBatchVertex v[4];
};

static std::vector<TerrainBatchQuad> s_terrainBatch;
static bool s_terrainBatchActive = false;
static std::vector<float> s_terrainBulkVerts;

static inline void TerrainBatch_Begin()
{
    s_terrainBatch.clear();
...
{
    TerrainBatchQuad q;
    q.textureId = (uint16_t)textureId;
    q.blendState = (uint8_t)blendState;
    q.primType = primType;
    for (int i = 0; i < 4; i++)
    {
        q.v[i].x = pos[i][0];
...
{
    if (s_terrainBatch.empty())
    {
        s_terrainBatchActive = false;
        return;
    }

    std::sort(s_terrainBatch.begin(), s_terrainBatch.end(),
        [](const TerrainBatchQuad& a, const TerrainBatchQuad& b)
        {
    ...
    s_terrainBulkVerts.clear();
    s_terrainBulkVerts.reserve(s_terrainBatch.size() * 36);
    ...
    auto flushBulk = [&]()
    {
        if (quadCount > 0 && !s_terrainBulkVerts.empty())
        {
            GL_DrawQuadsBulk(s_terrainBulkVerts.data(), quadCount);
            s_terrainBulkVerts.clear();
            quadCount = 0;
        }
    };

    for (const auto& q : s_terrainBatch)
    {
        if (q.blendState != curBlend || q.textureId != curTex)
        {
            flushBulk();

            if (q.blendState != curBlend)
            {
                switch ((TerrainBlendState)q.blendState)
                {
                case TB_OPAQUE:
                    DisableAlphaBlend();
                    break;
        ...
        for (int i = 0; i < 4; i++)
        {
            s_terrainBulkVerts.push_back(q.v[i].x);
            s_terrainBulkVerts.push_back(q.v[i].y);
            s_terrainBulkVerts.push_back(q.v[i].z);
            s_terrainBulkVerts.push_back(q.v[i].r);
            s_terrainBulkVerts.push_back(q.v[i].g);
            s_terrainBulkVerts.push_back(q.v[i].b);
            s_terrainBulkVerts.push_back(q.v[i].a);
            s_terrainBulkVerts.push_back(q.v[i].u);
            s_terrainBulkVerts.push_back(q.v[i].v);
        }
        ++quadCount;
    }
...
void CreateFrustrum2D(vec3_t Position)
{
 ...
 #if defined(__ANDROID__) || defined(MU_IOS)
    {
        ...
        if (SceneFlag == MAIN_SCENE && !CameraTopViewEnable && IsTerrainHeavyOverlayVisible())
        {
            const float overlayScale = 0.80f;
            WidthFar *= overlayScale;
            WidthNear *= overlayScale;
            CameraViewFar = (std::max)(2100.0f, CameraViewFar * overlayScale);
            CameraViewNear *= overlayScale;
            CameraViewTarget *= overlayScale;
        }
    }
#endif
