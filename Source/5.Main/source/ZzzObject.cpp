...
namespace
{
    ...
    constexpr float kObjectNearDistance = 1400.0f;
    constexpr float kObjectMidDistance = 2800.0f;
    constexpr float kObjectVeryFarDistance = 4300.0f;
    constexpr float kObjectHardCullDistance = 5600.0f;
    ...
    inline AdaptiveObjectDistanceThresholds GetObjectDistanceThresholds()
    {
        ...
        const float midDistance = kObjectMidDistance * zoomScale;
        const float veryFarDistance = kObjectVeryFarDistance * zoomScale;
        const float hardCullDistance = kObjectHardCullDistance * zoomScale;
    ...
    inline float GetObjectInstanceFrustumRange(const float baseRange)
    {
        ...
        float range = baseRange
            ...
            - extraWide * 120.0f
            - extraNarrow * 140.0f
            - (zoomRatio - 1.0f) * 125.0f
            - wideZoomWeight * 48.0f;
        const float minRange = baseRange - 220.0f;
        if (range < minRange)
        {
            range = minRange;
        }
    ...
    {
#if defined(__ANDROID__) || defined(MU_IOS)
        if (!IsObjectAdaptiveEnabled() || o == nullptr || !IsAdaptiveStaticSceneObject(o))
        {
            return false;
        }
        ...
        const bool stressedFrame = (currentFps < 26.0f);
        const bool severeFrame = (currentFps < 22.0f);
        ...
        if (bucket == AdaptiveDistanceBucket::Far)
        {
            if (crowdPressure > 84)
            {
                minImportance = 0.0036f;
            }
            if (crowdPressure > 112 && stressedFrame)
            {
                minImportance = 0.0064f;
            }
            if (crowdPressure > 148 && severeFrame)
            {
                minImportance = 0.0104f;
            }
        }
        else if (bucket == AdaptiveDistanceBucket::Mid)
        {
            if (crowdPressure > 132 && stressedFrame && distSq > nearAdaptiveMinDistanceSq)
            {
                minImportance = 0.0021f;
            }
            if (crowdPressure > 168 && severeFrame && distSq > nearAdaptiveMinDistanceSq * 1.5f)
            {
                minImportance = 0.0039f;
            }
        }
    ...
    inline float GetObjectVisibilityRange(const OBJECT* o, const float baseRange, const bool wasVisible)
    {
        ...
        if (wasVisible)
        {
            ...
            range -= 14.0f + GetWideZoomFrustumWeight() * 26.0f + extentBias * 0.22f;
        }
#else
        (void)wasVisible;
#endif
...
#if defined(__ANDROID__) || defined(MU_IOS)
        if (!IsObjectAdaptiveEnabled() || o == nullptr)
        {
            return;
        }
...
static bool RenderObjectWithAdaptiveBudget(OBJECT* o)
{
    ...
    const bool stressedFrame = (currentFps > 0.f && currentFps < 28.f);
    ...
    if (adaptiveEnabled)
    {
        ...
        if (bucket == AdaptiveDistanceBucket::Far)
        {
            ...
            {
                if (crowdPressure > 220)
                {
                    cullPeriod = severeFrame ? 3u : 2u;
                }
                if (crowdPressure > 300 && severeFrame)
                {
            ...
            {
                if (crowdPressure > 170 && severeFrame)
                {
                    cullPeriod = 2u;
                }
                if (crowdPressure > 240 && severeFrame)
                {
                    cullPeriod = 3u;
                }
            }
            else if (crowdPressure > 260 && severeFrame)
            {
                cullPeriod = 2u;
            }
        }
        ...
        {
            if (crowdPressure > 260 && severeFrame)
            {
                cullPeriod = 2u;
            }
            if (crowdPressure > 320 && severeFrame)
            {
                cullPeriod = 3u;
            }
        }
        ...
        if (edgeSensitiveObject || stabilizeFarObjectRender)
        {
            cullPeriod = 1u;
        }
    ...
    if (adaptiveEnabled)
    {
        ...
        if (bucket == AdaptiveDistanceBucket::Far)
        {
            if (distSq > thresholds.veryFarSq && crowdPressure > 120)
            {
                visualPeriod = 2u;
            }
            if (crowdPressure > 72 && stressedFrame)
            {
                visualPeriod = (std::max)(visualPeriod, uint32_t{2u});
            }
            if (crowdPressure > 96 && heavyFrame)
            {
        ...
        else if (bucket == AdaptiveDistanceBucket::Mid)
        {
            ...
            if (crowdPressure > 96 && severeFrame)
            {
                visualPeriod = sideOverlayVisible ? 2u : 3u;
            }
            if (crowdPressure > 140 && heavyFrame)
            {
                visualPeriod = 2u;
            }
            if (!sideOverlayVisible && crowdPressure > 220 && severeFrame)
            {
                visualPeriod = 3u;
            }
            else if (sideOverlayVisible && crowdPressure > 280 && severeFrame)
            {
                visualPeriod = 2u;
            }
        }
        else if (!sideOverlayVisible && bucket == AdaptiveDistanceBucket::Near && severeFrame && crowdPressure > 180)
        {
            visualPeriod = 2u;
        }
