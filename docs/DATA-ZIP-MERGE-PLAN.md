...
| Thành phần | Mô tả |
|------------|--------|
| `assets/data-patches/World74`, `World75` | Patch lá cây từ MuMain-5.2 |
...
**Output mong muốn:** `docs/takumi-manifests/TAKUMI-DATAZIP-AUDIT.csv` (đường dẫn, kích thước BASE/REF, quyết định copy/skip).
...
| Entry quan trọng | `unzip -l data.zip \| grep -E 'World7[45]/leaf|Dec2.dat'` |
...
| [`server-next/docs/DOCKER-BUILD-RUN.md`](../server-next/docs/DOCKER-BUILD-RUN.md) | Profile `datazip`, port 18080 |
...
| [`takumi-game-spec/GAMESERVER-DATA-FOLDER-MAP.md`](takumi-game-spec/GAMESERVER-DATA-FOLDER-MAP.md) | Data **server** (khác client zip) |
...
- [ ] Repack + copy `docker/data-zip/host/data.zip`.
- [ ] QA Android: char select + map 3 + relog.
