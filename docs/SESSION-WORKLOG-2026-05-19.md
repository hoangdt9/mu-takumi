...
| Done | Wrapper tương thích ngược: `./scripts/docker-stack.sh` → `docker/docker-stack.sh` |
...
| **Deploy** | `cd server-next && ./scripts/docker-stack.sh --host-build --recreate --detach` |
...
| Done | `assets/data-patches/README.md`, `docs/DATA-ZIP-MERGE-PLAN.md` (bảng size + verify). |
...
cd server-next
./scripts/apply-sql.sh "$TAKUMI_PG_CONNECTION_STRING" sql/patches/013_test_account_mg001_seed.sql
...
- `docs/DATA-ZIP-MERGE-PLAN.md`, `docs/SESSION-WORKLOG-2026-05-19.md`
