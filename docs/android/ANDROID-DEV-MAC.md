...
- Spec đầy đủ: [`docs/game-spec/SKILL-HOTKEY-PERSISTENCE.md`](game-spec/SKILL-HOTKEY-PERSISTENCE.md).
- **Skill combat MG (cast, damage, Linh hồn VFX):** [`docs/android/MOBILE-SKILL-COMBAT-GUIDE.md`](MOBILE-SKILL-COMBAT-GUIDE.md) · QA [`docs/qa/M9-mg-skill-combat.md`](qa/M9-mg-skill-combat.md).
...
- **Tuỳ chọn `data.zip` qua LAN:** cùng file compose, profile **`datazip`** — nginx publish cổng host **mặc định 18080** (khác **44605/44606/54444**). File đặt tại `takumi/docker/data-zip/host/data.zip`. Bật: `docker compose --profile datazip up -d` hoặc `cd server-next && ./scripts/docker/docker-stack.sh --detach` (mặc định đã gộp datazip), hoặc `COMPOSE_PROFILES=datazip` trong `.env`. **Không** chạy đồng thời `datazip` của `server-next` và `takumi/docker` nếu cùng publish port **18080**.
...
```bash
./server-next/scripts/smoke/check-takumi-ports.sh
```

- **Server LAN trong Docker:** trong `server-next` chạy `docker compose up -d` hoặc `./scripts/docker/docker-stack.sh --detach` — stack gồm **Postgres** và **LegacyLoginHost** (Connect **44605** + login **44606**). Cần `.env` với **`TAKUMI_LAN_IP`** = IP LAN Mac.
...
3. Đối chiếu `server-next/docs/milestones/IMPLEMENTATION-CHECKLIST.md` (MVP: một session login TCP vs **game port** sau select — nếu checklist ghi “chưa có”, client có thể thoát sau select).
...
**Kỹ thuật:** `NewMoveCharacterScene` chạy **trước** `CInput::Update()` trong cùng frame nên không dùng `IsLBtnDn()` cho nhánh này; dùng `SEASON3B::IsPress` / `IsRepeat(VK_LBUTTON)`. Chi tiết touch + ray pick + IME sau login: **`docs/journal/DEVELOPMENT-LOG-2026-05-12.md`**. Bổ sung **SysMenu / modal / xóa nhân vật / JNI IME Done / thứ tự `UpdateMouseFromTouch`**: **`docs/journal/DEVELOPMENT-LOG-2026-05-14.md`**. **In-world combat, shop, inventory, level-up VFX:** **`docs/journal/DEVELOPMENT-LOG-2026-05-16.md`**, **`docs/journal/DEVELOPMENT-LOG-2026-05-17.md`**.
