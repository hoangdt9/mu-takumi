...
Ghi lại các hạng mục **đã merge trên `main`** (xem `git log` để chi tiết diff). Dùng kèm **`server-next/docs/milestones/IMPLEMENTATION-CHECKLIST.md`**.
...
- **`./scripts/docker/docker-stack.sh`**: pull, up, force-recreate `legacy-login` + `game-host`; flags `--detach`, `--host-build`, `--recreate`.
...
**Docs liên quan:** `server-next/docs/docker/DOCKER-BUILD-RUN.md`, `server-next/docs/character/M6-GAME-TCP-CHECKLIST.md`, `server-next/docs/character/M7-CHARACTER-PERSISTENCE-CHECKLIST.md`.
