...
**Đọc song song:** checklist pha và gate lộ trình: [`docs/migration/TAKUMI-MIGRATION-OPENMU-CHECKLIST.md`](TAKUMI-MIGRATION-OPENMU-CHECKLIST.md).  
...
| §1 — Manifest snapshot | **Xong** — tiêu đề `# Snapshot` + `# Git SHA…` đầu mỗi file trong [`docs/manifests/`](manifests/) |
| §2 — Macro / build 603 | **Xong** — [`docs/game-spec/SEASON-AND-DEFINES.md`](game-spec/SEASON-AND-DEFINES.md) |
| §3 — Coupling `Source/Util/` (theo vcxproj) | **Xong** — ghi trong `SEASON-AND-DEFINES.md` |
| §5 (SQL) — Trích proc/bảng từ C++ | **Xong** — [`docs/game-spec/TAKUMI-SQL-BACKLOG.md`](game-spec/TAKUMI-SQL-BACKLOG.md) |
| §11 / Phase 2 — `SQLUp.sql` + map OpenMU (concept) | **Khung** — [`PHASE2-OPENMU-DATA-MODEL-MAP.md`](game-spec/PHASE2-OPENMU-DATA-MODEL-MAP.md); inspect + restore **đã chạy dev** (snap OPENMU checklist 2026-05); **mapping đầy đủ + ETL + Gate 2** TODO |
| §8 — Drift `Sub 1/Data` vs `Data`; **`4.GameServer` vs `4.GameServer_real`** | **Xong** — [`DATA-SUB1-DRIFT.md`](game-spec/DATA-SUB1-DRIFT.md), [`GAMESERVER-VS-GAMESERVER-REAL.md`](game-spec/GAMESERVER-VS-GAMESERVER-REAL.md) |
| §4 — `1.ConnectServer` vs `1.ConnectServer_real` | **Xong** (INI đồng nhất) — [`CONNECT-SERVER-REAL-DRIFT.md`](game-spec/CONNECT-SERVER-REAL-DRIFT.md) |
| §7–§8a — Ánh xạ thư mục `Data/` | **Khung** — [`GAMESERVER-DATA-FOLDER-MAP.md`](game-spec/GAMESERVER-DATA-FOLDER-MAP.md) |
| §17 — Tracker mẫu (sheet/issue) | **Khung** — [`MANIFEST-TRACKER-TEMPLATE.md`](MANIFEST-TRACKER-TEMPLATE.md) |
| §12 — Batch / docker / script | **Xong** — [`docs/migration/OPERATIONS-MIGRATION-NOTES.md`](OPERATIONS-MIGRATION-NOTES.md) |
...
| Android Gradle + `server-next/.env` / `env.defaults` (BuildConfig bootstrap, `data.zip` URL) | **Xong (2026-05-14)** — `Source/android/app/build.gradle`, `docs/android/ANDROID-DEV-MAC.md` § chuyển Mac |

| Mục | Trạng thái |
|-----|------------|
| Android/iOS — màn chọn nhân vật, double-tap / long-press → `StartGame`, ray pick, IME sau login | **Đã làm (commit ~2026-05-13)** — [`docs/journal/DEVELOPMENT-LOG-2026-05-12.md`](DEVELOPMENT-LOG-2026-05-12.md) |
| Android/iOS — IME toàn cục: `ShowWin` active cùng frame, đồng bộ touch→Mouse trước handler, Init `SinglePasswd` trên Android, SysMenu tap-outside, JNI **Done** → Return, flow xóa nhân vật + captcha 6 số | **Đang trong working tree (ghi 2026-05-14)** — [`docs/journal/DEVELOPMENT-LOG-2026-05-14.md`](DEVELOPMENT-LOG-2026-05-14.md) |
...
| File | Nội dung |
|------|----------|
| [`docs/manifests/TAKUMI-SERVER-SOURCE-MANIFEST.txt`](manifests/TAKUMI-SERVER-SOURCE-MANIFEST.txt) | Mọi `*.cpp` server `1.Connect` … `4.Game` + **`6.GetMainInfo`** (~385 file nguồn; GameServer ~281). |
| [`docs/manifests/TAKUMI-SERVER-HEADERS-MANIFEST.txt`](manifests/TAKUMI-SERVER-HEADERS-MANIFEST.txt) | Mọi `*.h` / `*.hpp` trong cùng prefix (đọc ghép đôi với `.cpp`). |
| [`docs/manifests/TAKUMI-MUSERVER-GAMEDATA-FILES.txt`](manifests/TAKUMI-MUSERVER-GAMEDATA-FILES.txt) | Danh sách **từng file** dưới `MuServer/4.GameServer/Data/` (bundle nội dung game trong repo). |
| [`docs/manifests/TAKUMI-MUSERVER-CONFIG-MANIFEST.txt`](manifests/TAKUMI-MUSERVER-CONFIG-MANIFEST.txt) | `*.ini` / `*.txt` / `*.dat` / `*.bat` / `*.sql` tại các service + runtime GameServer (**không** gồm toàn bộ bulk `Data/`, không gồm log). |
...
# Script đầy đủ: docs/manifests/README.md
```

- [x] Gắn **snapshot + `# commit:`** vào đầu mỗi manifest (`git rev-parse HEAD` sau mỗi lần tái sinh); hướng dẫn: [`docs/manifests/README.md`](manifests/README.md).
...
- [x] Đối chiếu **define preprocessor / SDK version** trong từng `.vcxproj` với nhãn fork OpenMU — **đã tóm tắt:** [`docs/game-spec/SEASON-AND-DEFINES.md`](game-spec/SEASON-AND-DEFINES.md).
...
- [x] **`Source/Util/`** — `cryptopp`, `detours`, `lua`, `mapm`: **theo `.vcxproj` server** Join dùng `MD5.*`, Game dùng `CCRC32`/`Math`; các thư khác không được kéo trực tiếp — chi tiết [`SEASON-AND-DEFINES.md`](game-spec/SEASON-AND-DEFINES.md). (Backlog: grep thư viện ẩn nếu linker dùng `.lib` không khai trong vcxproj.)
...
- [x] Cặp **`ConnectServer.ini`**, **`ServerList.ini`** (`MuServer/1.ConnectServer*` và `1.ConnectServer_real`): cổng đã vào [`TAKUMI-SERVER-NETWORK-BASELINE.md`](protocol/TAKUMI-SERVER-NETWORK-BASELINE.md); **`_real`** vs chuẩn: **cùng nội dung INI** @ 2026-04-30 — [`CONNECT-SERVER-REAL-DRIFT.md`](game-spec/CONNECT-SERVER-REAL-DRIFT.md) (khác chủ yếu tên/binary deploy).
...
- [x] Trích **`QueryManager.cpp` + mọi `ExecQuery`/`EXEC` trong Data/Join `*.cpp`** — **đã lưu:** [`docs/game-spec/TAKUMI-SQL-BACKLOG.md`](game-spec/TAKUMI-SQL-BACKLOG.md) (62 proc + 51 bảng heuristic).
...
Danh sách **đầy đủ từng đường dẫn** nằm trong [`docs/manifests/TAKUMI-SERVER-SOURCE-MANIFEST.txt`](manifests/TAKUMI-SERVER-SOURCE-MANIFEST.txt), mục `## 4.GameServer GameServer/*.cpp`.
...
- [x] **Khung `Data/*` theo chức năng** (discovery, không thay manifest từng file): [`GAMESERVER-DATA-FOLDER-MAP.md`](game-spec/GAMESERVER-DATA-FOLDER-MAP.md).
...
- [x] **Hai shard chạy thật** (`GameServerCS` + `Sub 1`): **EXE, path outdir `.vcxproj`, port game, ServerCode** → [`protocol/TAKUMI-SERVER-NETWORK-BASELINE.md`](protocol/TAKUMI-SERVER-NETWORK-BASELINE.md) và [`SEASON-AND-DEFINES.md`](game-spec/SEASON-AND-DEFINES.md). Map sang **multiple game servers** trong OpenMU vẫn **TODO** fork.
...
- [x] **Đã có danh sách đường dẫn từng file** trong [`TAKUMI-MUSERVER-GAMEDATA-FILES.txt`](manifests/TAKUMI-MUSERVER-GAMEDATA-FILES.txt) — checkbox dưới đây là **đọc/import** theo từng hệ subsystem (kéo backlog); **bản đồ thư mục** (chức năng ↔ OpenMU): [`GAMESERVER-DATA-FOLDER-MAP.md`](game-spec/GAMESERVER-DATA-FOLDER-MAP.md).
...
- [x] **`MuServer/4.GameServer/Sub 1/Data/`**: `diff -rq` với **`MuServer/4.GameServer/Data/`** — **không chênh** @ 2026-04-30; ghi trong [`docs/game-spec/DATA-SUB1-DRIFT.md`](game-spec/DATA-SUB1-DRIFT.md).
...
- [x] Diff với **`4.GameServer`** — **ghi:** [`docs/game-spec/GAMESERVER-VS-GAMESERVER-REAL.md`](game-spec/GAMESERVER-VS-GAMESERVER-REAL.md). **Authority tree:** `4.GameServer` (đủ CS + Sub 1); `_real` chủ yếu khác layout / localhost / tuning (`Common.ini`, `GHRSReset.ini`, `MapServerInfo.dat`).
...
- [ ] **`MuOnline.bak`** — nguồn schema + dữ liệu thật để spike ETL Postgres (**chân lý** so với `SQLUp.sql` — đã map patch trong [`PHASE2-OPENMU-DATA-MODEL-MAP.md`](game-spec/PHASE2-OPENMU-DATA-MODEL-MAP.md)).
- [x] **`SQL Back/*.sql`** — **đã tóm trong** [`docs/game-spec/TAKUMI-SQL-BACKLOG.md`](game-spec/TAKUMI-SQL-BACKLOG.md) (mục **SQL Back**); vẫn cần diff chi tiết DDL `SQLUp.sql` ↔ `.bak` / OpenMU khi vào Gate 2.
- [x] **Trích proc/bảng từ server C++** (bổ sung cho `.bak`) — [`docs/game-spec/TAKUMI-SQL-BACKLOG.md`](game-spec/TAKUMI-SQL-BACKLOG.md).
- [x] **Schema inspect (read-only)** — [`takumi-mssql-inspect` / `takumi-pg-inspect`](../tools/db-migrate/README.md), solution [`Takumi.DbTools.slnx`](../tools/db-migrate/dotnet/Takumi.DbTools.slnx), template [`PHASE2-MAPPING-TEMPLATE.csv`](game-spec/PHASE2-MAPPING-TEMPLATE.csv).
- [x] **`docker/sql/restore-muonline.sh`** — **đã có trong repo**; restore `.bak` vào container `takumi-mssql` (profile `db`), hướng dẫn [`docker/README.md`](../docker/README.md) + ETL quy ước [`tools/db-migrate/README.md`](../tools/db-migrate/README.md). *(Gate 2: cần chạy thật trên máy dev có `.bak` + Docker.)*
...
| Path | Kiểm tra |
|------|----------|
| [x] `MuServer/Start_*.bat`, `Stop_MuServer.bat`, … | **Đã ghi** thứ tự + hướng OpenMU — [`docs/migration/OPERATIONS-MIGRATION-NOTES.md`](OPERATIONS-MIGRATION-NOTES.md) |
...
- [x] Dùng [`TAKUMI-SERVER-HEADERS-MANIFEST.txt`](manifests/TAKUMI-SERVER-HEADERS-MANIFEST.txt); khi audit một `*.cpp`, đánh dấu **`*.h`** include đã đọc.
- [x] **`stdafx.h` / pch / macro season** — tóm trong [`docs/game-spec/SEASON-AND-DEFINES.md`](game-spec/SEASON-AND-DEFINES.md) (mở rộng cờ `CUSTOM_*` sau).
...
**Kết:** Checklist trong file này + **manifest** trong `docs/manifests/` là **bộ đầy đủ** để không bỏ sót artifact Takumi trong quá trình chuyển sang OpenMU. Lộ trình theo gate thời gian vẫn dùng [`TAKUMI-MIGRATION-OPENMU-CHECKLIST.md`](TAKUMI-MIGRATION-OPENMU-CHECKLIST.md); **§11 / Phase 2** (MSSQL restore, inspector, mapping CSV) giữ đồng bộ với checklist pha đó. **Server Next (LegacyLoginHost) — bước kế tiếp ưu tiên QA/engineering:** xem [`server-next/docs/milestones/IMPLEMENTATION-CHECKLIST.md`](../server-next/docs/milestones/IMPLEMENTATION-CHECKLIST.md) mục **Planned next steps (consolidated)**.
