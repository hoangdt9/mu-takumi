...
| `docs/milestones/IMPLEMENTATION-CHECKLIST.md` | Feature and QA checklist for Connect / Login / character flow. |
| `docs/protocol/M1-PROTOCOL-PARITY-MAP.md` | **M1** inventory: client `TranslateProtocol` / `ProtocolCoreEx` vs `LegacyLoginHost` coverage. |
| `docs/protocol/LOGIN-WIRE-FORMAT.md` | Encrypted login wire notes + link to M1 map. |
| `docs/character/M4-WORLD-POSITION-CHECKLIST.md` | **M4** world spawn + JSON ↔ Postgres roster (tick từng mục khi xong). |
| `docs/character/M4-TILE-AND-COORDINATES.md` | **M4** hợp đồng tọa độ: tile `byte` / wire walk — không float world trong iteration này. |
| `docs/character/M4-ROSTER-SSOT.md` | **M4** quyết định nguồn sự thật roster (JSON + mirror Postgres; Postgres-only = backlog). |
| `docs/character/M5-JOIN-HANDOFF-CHECKLIST.md` | **M5** F4 03 advertised game port + in-memory session ticket (prep for M6). |
| `docs/character/M6-GAME-TCP-CHECKLIST.md` | **M6** dedicated game TCP scaffold (`GameListenHost`, `GameHost`, compose profile `gamehost`). |
| `docs/character/M7-CHARACTER-PERSISTENCE-CHECKLIST.md` | **M7** HP/MP/zen + join restore; SQL `004_*` + roadmap C#/JSON. |
| `docs/world/M8-M10-WORLD-RUNTIME-CHECKLIST.md` | **M8–M10** ETL thế giới, NPC, movement/broadcast (sau login). |
| `docs/docker/DOCKER-BUILD-RUN.md` | **Docker QA:** build/run stack, `docker-stack.sh`, khi nào recreate vs build image vs APK. |
...
**M5 (join handoff):** optional **`TAKUMI_GAME_PORT`** overrides the port embedded in Connect Server **`C1 F4 03`** (defaults to **`TAKUMI_LOGIN_PORT`**). Session tickets are issued in-memory after login (`Takumi.Server.Join`); see **`docs/character/M5-JOIN-HANDOFF-CHECKLIST.md`**.

**M6 (game TCP):** optional split port **`TAKUMI_GAME_PORT`** + run **`Takumi.Server.GameHost`** (or Docker **`--profile gamehost`** / **`./scripts/docker/docker-stack.sh --detach --with-gamehost`**). With **`TAKUMI_ACCOUNTS`** + **`TAKUMI_SERVER_SERIAL`** (from **`env.defaults`** or `.env`), GameHost runs **minimal-login** (same roster JSON as LegacyLoginHost). See **`docs/character/M6-GAME-TCP-CHECKLIST.md`**.

## Quick start (database + LAN server in Docker)

**Chi tiết (tiếng Việt):** **`docs/docker/DOCKER-BUILD-RUN.md`** — ma trận “khi nào recreate / pull / build APK”, profile `datazip` / `gamehost`, xử lý sự cố.
...
2. **`./scripts/docker/docker-stack.sh --detach`** (khuyên dùng) hoặc `docker compose up -d` — **Postgres** (**54444**) + **LegacyLoginHost** (**44605** / **44606**). Script mặc định **force-recreate** `legacy-login` (và `game-host` nếu `TAKUMI_GAME_PORT` > 0) để `dotnet build` lại từ bind-mount; đợi log **`[legacy-login] build OK`**. Dec2 trong container: **`/keys/Dec2.dat`** (`./keys` → `/keys`). **`TAKUMI_DEC2_PATH` trong `.env` chỉ cho `dotnet run` trên host**, không map path macOS vào container.
...
Port/env sanity: `./scripts/smoke/check-takumi-ports.sh` from `server-next`.

## Postgres: apply `sql/init` when the Docker volume already exists

Compose mounts `./sql/init` into **`/docker-entrypoint-initdb.d`** — scripts run **only on first database init**. If the volume was created before a new file appeared in `sql/init/`, apply manually (recommended: run **`./scripts/db/apply-sql.sh`** so **`001_*.sql`** then **`002_*.sql`** both apply).
...
Clears logcat by default; use `TAKUMI_LOGCAT_CLEAR=0 ./scripts/android/watch-android-takumi-log.sh` to keep history. Requires `adb` and a connected device.

See also `../docs/android/ANDROID-DEV-MAC.md` (Takumi Server Next + optional `datazip` profile under `../docker/`).
