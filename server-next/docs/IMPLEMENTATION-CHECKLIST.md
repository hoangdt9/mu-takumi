...
Last updated: 2026-05-15 (roster JSON vs DB clarification in checklist)
...
- [ ] **M4b–M5b (remaining):** Postgres-first roster **SSOT** beyond JSON mirror — **đóng đúng câu hỏi “sao không chỉ DB”**: bỏ phụ thuộc chỉnh tay / rebuild từ `takumi-roster/*.json` cho `posX`/`posY`/`mapId`; đồng bộ domain `character` + importer world columns — see **`docs/M4-WORLD-POSITION-CHECKLIST.md`** §Importer / SSOT (M4b §*SSOT Postgres-only*); does **not** block M5 join/ticket work.
...
4. **M4 — Login + nhân vật + vị trí lưu thế giới** *(M4a + M4c walk + periodic save + **M4b mirror + merge health metrics** — SSOT Postgres-only + tile/float still open; see **`docs/M4-WORLD-POSITION-CHECKLIST.md`** §Handoff M5)*  
   - [x] Bảng / roster: `map_id`, `pos_x`, `pos_y`, `angle` — **M4a:** file **`server-next/takumi-roster/<accountId>.json`** (vd. `test.json` cho account `test`); **M4b:** mirror bảng `character_roster` khi `TAKUMI_ROSTER_DB_SYNC`; join dùng `JoinMapSpawnWire`, không hard-code trong `Program` ngoài env default. *(Thuộc mục này — không phải bug; SSOT chỉ DB = mục `[ ]` trong **`## Next (High Priority)`**.)*  
...
9. **M9 — NPC & monster runtime** *(scope A — **`docs/M9-NPC-MONSTER-CHECKLIST.md`**)*  
