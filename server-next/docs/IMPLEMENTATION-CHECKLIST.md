...
|--------|--------------------------|-------------------------|----------------|
| Only `server-next` C#, compose, `.env`, SQL, keys on host | **No** | **No** | `docker compose` **up / recreate** the stack; set **`TAKUMI_LAN_IP`** (optional override **`TAKUMI_PUBLIC_HOST`**) to the IP the **phone** can reach (same LAN as `44605` / login port). |
