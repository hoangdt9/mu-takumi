...
| — | SSOT Postgres-only (bỏ JSON) | — | **OPEN** — ticket M4b lớn |

### B. Vitals & join stats (M7)

| Legacy | Hành vi | `server-next` | Ghi chú |
|--------|---------|---------------|---------|
| `GCLifeSend` / mana | `0x26` / `0x27` | `LifeManaWire602`, `RosterVitalsOutboundTracker` | **Partial** — track outbound; combat chưa đẩy đủ |
...
| `CGItemGetRecv` | `0x22` pick up | — | **OPEN** — M7+ |
| `CGItemDropRecv` | `0x23` drop | — | **OPEN** |
| `CGItemMoveRecv` | `0x24` move | — | **OPEN** |
...
2. **M7d combat vitals** — hook damage/heal → `current_hp` / outbound `0x26` (đọc `GCLifeSend` trong `ItemManager.cpp` / combat path).
3. ~~**Inventory persist (M4+M7)**~~ — **Done:** `InventorySlotMirrorWriter`, `PostgresInventorySlotRepository` write, flush disconnect.
4. **Item world ops** — `0x22`–`0x24` pick/drop/move (đọc `Protocol.cpp` case + `ItemManager`); tách PR nhỏ.
