...
- **Fix (pick one):**
  ...
  2. **Split stack:** keep **`TAKUMI_GAME_PORT=55901`** and start **`GameHost`** — e.g. `./scripts/docker-stack.sh --detach` (auto-enables **`gamehost`** when `TAKUMI_GAME_PORT` is a positive integer), or `docker compose --profile gamehost up -d`, or `./scripts/docker-stack.sh --detach --with-gamehost` if the port is unset but you still want the container.
...
If a local **`.env`** contains **`TAKUMI_PUBLISHED_SERVERS`** (or similar ad-hoc keys), note that **`server-next` does not read that variable** — Connect sub-server rows come from the built-in **F4 06** builder / BMD alignment (`docs/M3-CONNECT-BMD.md`, **`TAKUMI_CS_CONNECT_*`**). Remove unused keys to avoid confusion.
...
If **`TAKUMI_GAME_PORT`** is **unset** but you still want **`game-host`** for experiments: `./scripts/docker-stack.sh --detach --with-gamehost`.
...
1. **`errno=111` trên cổng game (ví dụ 55901)**  
   Chưa có process lắng nghe đúng cổng F4 03. Chạy `./scripts/docker-stack.sh --detach` (hoặc `docker compose --profile gamehost up -d`, hoặc `./scripts/docker-up.sh --with-gamehost`). Nếu chỉ `legacy-login` mà `.env` vẫn set `TAKUMI_GAME_PORT` khác `TAKUMI_LOGIN_PORT` → client sẽ bị refused sau chọn server.
