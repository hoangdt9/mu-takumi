...
Last updated: 2026-05-17 — shop F3 E9/ED, inventory `0x24` sync: **`../../docs/journal/DEVELOPMENT-LOG-2026-05-17.md`**.  
**Owner nhánh đề xuất:** `mac-m1` (M4/M7); **không** sửa `WorldGameplayHandlers` / `ShopCommerceHandler` trừ bug — xem **`combat/M9-M8-NPC-GAMEPLAY-OWNERSHIP.md`**.

**Checklist chi tiết:** `character/M4-WORLD-POSITION-CHECKLIST.md`, `character/M7-CHARACTER-PERSISTENCE-CHECKLIST.md`, `character/M4-ROSTER-SSOT.md`, `character/M4-TILE-AND-COORDINATES.md`.
...
| `ObjectManager` `CharacterMakePreviewCharSet` | 17-byte equip preview | `CharacterListEquipPreview602` | **Done** (2026-05-18) — wear slot 0–8 → CharSet; wing nibble `itemSub-35`; tests + mg001 seed |
...
| `2.DataServer` character tables | SSOT SQL | `character_domain` + `character_staging` importer | **Done** (minimal hosts); EF full host ngoài scope |
| — | Postgres mirror | `character_roster`, `CharacterRosterMirrorWriter` | **Done** |
| — | SSOT Postgres-only (bỏ JSON) | `CharacterRosterBootstrap`, `TAKUMI_ROSTER_DB_PRIMARY`, `CharacterDomainMirrorWriter` | **Done** — DB-first + domain mirror |

### B. Vitals & join stats (M7)

| Legacy | Hành vi | `server-next` | Ghi chú |
|--------|---------|---------------|---------|
| `GCLifeSend` / mana | `0x26` / `0x27` | `LifeManaWire602`, `RosterVitalsOutboundTracker`, `RosterVitalsCombat` | **Partial** — monster/PvP hit → `0x26` (HP+SD), SD trước HP; DB `current_shield`/`max_shield`; join `F3 03` + outbound tracker parse shield |
...
| `CGItemMoveRecv` | `0x24` move | `ItemWorldHandler`, `InventoryBagGrid`, footprints | **Partial** — inv+wear + `0x24`→`F3 10` sync (`6330de9`); trade/warehouse **OPEN** |
| `GCItemValueSend` | `F3 E9` | `ShopItemValueSender`, `ShopItemValueResolver` | **Done** (`1d77d94`) |
| Buy confirm | `F3 ED` | `ShopCommerceHandler` + client `ReceiveBuyConfirm` | **Done** when `TAKUMI_SHOP_BUY_CONFIRM=1` (`65e0e36`) |
| `CGItemUseRecv` | `0x26` use | `ItemWorldHandler`, `InventoryConsumableRules` | **Partial** — HP/MP/SD potions; scroll/jewel/trade **OPEN** |
...
| `CGItemRepairRecv` | `0x34` | `ShopCommerceHandler` | **Done** — NPC + self-repair (`repairType` 0/1), `ConvertRepairGold` parity, `F3 10` sync, `Item.txt` max durability |
...
4. ~~**Item world ops**~~ — **Partial:** `0x22`–`0x24` + footprints + Android plain `C4` (`6330de9`); trade/warehouse/equip rules **OPEN**.
...
3. Thêm một dòng vào **`milestones/WORKSTREAM-OWNERSHIP.md`** § “Đã xong trên main”.
