...
**Tham chiếu code:** `JoinMapSpawnWire`, `JoinMapServerWire602`; roster JSON + **`TakumiPostgresMirror`**; SQL `001`–`004` trong `sql/init/`. **Hợp đồng tọa độ / SSOT:** `character/M4-TILE-AND-COORDINATES.md`, **`character/M4-ROSTER-SSOT.md`**.  
**Migration `Source/` → server-next (M4 item + M7):** **`character/M4-M7-CHARACTER-ITEM-MIGRATION.md`**.

**Định tuyến M7+ (không thuộc close M4):** HP/MP/zen persist + join wire từ DB/JSON → **`character/M7-CHARACTER-PERSISTENCE-CHECKLIST.md`**. ETL thế giới tĩnh, NPC, broadcast scope → **`world/M8-M10-WORLD-RUNTIME-CHECKLIST.md`**. M4 giữ vai trò **roster vị thế + mirror + walk tile**; các mục `[ ]` còn lại ở đây là SSOT Postgres-only / `character` domain / listener GS đầy đủ.
...
- [x] (Tuỳ chọn) Job CI / script `scripts/db/apply-sql.sh` idempotent.
...
- [x] **Quyết định SSOT (tài liệu, M4 freeze):** JSON authoritative + Postgres mirror + overlay merge — không Postgres-only cho tới khi có dự án riêng; **`character/M4-ROSTER-SSOT.md`**. Dev **M5** có thể làm ticket/handoff trước khi SSOT code xong.
...
- [x] Chuẩn hóa tile BYTE vs tọa độ float world — **hợp đồng M4:** toàn bộ wire + roster + `character_roster` dùng **tile 0–255** (`byte` / cast `smallint`); float / sub-tile **ngoài phạm vi** roster iteration này; tài liệu **`character/M4-TILE-AND-COORDINATES.md`** + XML doc `CharacterRosterRow` / `JoinMapSpawnWire` / `ClientWalkPackets602`. **Vitals / zen** trên join wire: **`character/M7-CHARACTER-PERSISTENCE-CHECKLIST.md`**.
- [ ] Listener **process** game-only (M6 `GameHost`) parity đầy đủ với `GameServer` (scope, broadcast, …) — **owner: M6+ / M8–M10** — **`world/M8-M10-WORLD-RUNTIME-CHECKLIST.md`** §M9–M10; không gộp vào close M4.
...
**M5 làm tiếp (không chờ M4 SSOT):** `Takumi.Server.Join`, **`TAKUMI_GAME_PORT`**, Postgres **`session_ticket`**, **`TAKUMI_SESSION_HANDOFF_DB`**, optional **`F1 A5`/`F1 A6`** — chi tiết **`character/M5-JOIN-HANDOFF-CHECKLIST.md`**. Android split TCP smoke: **`character/M6-GAME-TCP-CHECKLIST.md`**.

**Không nằm trong M5:** bảng `character` “chuẩn” + importer world-only, broadcast scope — **`character/M7-CHARACTER-PERSISTENCE-CHECKLIST.md`** + **`world/M8-M10-WORLD-RUNTIME-CHECKLIST.md`**. **Tile vs float:** đã chốt hợp đồng tile-only trong **`character/M4-TILE-AND-COORDINATES.md`** (M4 không dùng float world).

---

## M5+ (liên quan, không thuộc M4)

- [x] Session ticket + handoff cổng game (**M5**) — **`character/M5-JOIN-HANDOFF-CHECKLIST.md`** (`TAKUMI_GAME_PORT`, `Takumi.Server.Join`).
