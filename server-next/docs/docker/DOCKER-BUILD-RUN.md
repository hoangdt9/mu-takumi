...
**Liên quan:** `../README.md`, `../docker-compose.yml`, `../scripts/docker/docker-stack.sh`, `../../docs/android/ANDROID-DEV-MAC.md`, `M6-GAME-TCP-CHECKLIST.md`.
...
| **Lệnh hàng ngày** | `./scripts/docker/docker-stack.sh` — pull image + `up` + **force-recreate** `legacy-login` (và `game-host` nếu bật). |
...
Kiểm tra port: `./scripts/smoke/check-lan-connect-ports.sh` hoặc `./scripts/smoke/check-takumi-ports.sh`.
...
# 4) Port trên Mac
./scripts/smoke/check-lan-connect-ports.sh
./scripts/smoke/smoke-connect-from-host.sh 192.168.x.x 44605   # từ Mac → phải OK
...
```bash
./scripts/docker/docker-stack.sh --no-gamehost --host-build --recreate --detach
```
...
Chi tiết client: `../../docs/android/ANDROID-DEV-MAC.md`. Wire game: `M6-GAME-TCP-CHECKLIST.md`.
...
```bash
./scripts/docker/docker-stack.sh              # pull + up + recreate legacy-login (+ game-host) + tail log
./scripts/docker/docker-stack.sh --detach     # giống trên, không bám log (khuyên dùng sau pull code)
```
...
**Ghi chú client Android:** sau `git pull` server + client, rebuild APK (`main` ≥ `57f37a3` cho FPS/combat fixes) — xem **`../../docs/journal/DEVELOPMENT-LOG-2026-05-16.md`**.
...
Rồi `./scripts/docker/docker-stack.sh --detach` (script tự merge profile **gamehost**).
...
```bash
./scripts/host/run-legacy-login-host.sh    # dotnet watch LegacyLoginHost
```
...
|-------------|------------------|
| App không kết nối | `./scripts/smoke/check-lan-connect-ports.sh`; IP `.env` = IP Mac; tắt OpenMU trùng cổng |
...
| Sửa code mà hành vi cũ | Chưa recreate → `./scripts/docker/docker-stack.sh --detach`; đợi `build OK` |
...
| DB thiếu bảng | `./scripts/db/apply-sql.sh` (volume cũ) |
...
- **Hàng ngày (sửa server):** `cd server-next && ./scripts/docker/docker-stack.sh --detach` → đợi `build OK` → test APK.
