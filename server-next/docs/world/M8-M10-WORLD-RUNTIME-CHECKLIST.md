...
**Phụ thuộc:** **M7** (`character/M7-CHARACTER-PERSISTENCE-CHECKLIST.md`) cho stats persist; **M6** (`character/M6-GAME-TCP-CHECKLIST.md`) cho listener game TCP; **M1** cho opcode map.  
**Nhật ký gần đây:** **`../../docs/journal/DEVELOPMENT-LOG-2026-05-17.md`**.
...
- [x] ETL: **`MonsterSpawnDbImporter`** + **`scripts/db/import-monster-spawn.sh`** (cần `TEST_PG` / `TAKUMI_PG_*` + `TAKUMI_MONSTER_SET_BASE_PATH`).
...
- [x] **Move map dev:** **`world/M8-MOVE-MAP-PARITY-CHECKLIST.md`** (P0–P4, P6.1–P6.2, `smoke-m8.sh`).
...
Chi tiết: **`combat/M9-NPC-MONSTER-CHECKLIST.md`**, **`combat/M9-MONSTER-AI-PORT-CHECKLIST.md`**.
...
- [ ] Quyết định Postgres-only vs bridge MSSQL legacy — **`milestones/IMPLEMENTATION-CHECKLIST.md`** §11; API nội bộ cho `Takumi.Server.Game`.
