...
```bash
cd server-next
./scripts/spawn/sync-monster-spawn-stack.sh   # hoặc từng bước:
# ./scripts/spawn/sync-all-spawns-from-openmu.py
# ./scripts/spawn/merge-spawns-from-references.py
# ./scripts/spawn/report-spawn-sources.py
...
| P0.3 | Chạy `./scripts/db/import-monster-spawn.sh` (hoặc `import-world-static-data.sh`) | [x] | `./scripts/spawn/sync-monster-spawn-stack.sh`; bật `TAKUMI_MONSTER_SPAWN_DB=1` nếu dùng DB |
| P0.4 | Log startup: không còn “Lorencia fallback only” trên Docker prod | [ ] | Chỉ fallback khi thiếu file |
| P0.5 | `./scripts/spawn/report-monster-spawn-coverage.sh` pass | [x] | Unit + in ra map counts |
...
| P1.2 | Noria (3), Devias (2), Lorencia (0) smoke | [~] | Devias section 1 từ OpenMU075; QA: [`M8-MOVE-WARP-MONSTER-QA.md`](./qa-gates/M8-MOVE-WARP-MONSTER-QA.md) |
...
| P1.5 | Drift vs **OpenMU** SeasonSix | [~] | `./scripts/spawn/compare-spawn-openmu.sh` |
...
```bash
./scripts/spawn/enable-move-map-field-spawns.sh
```

**Bước 2** — thêm spot từ OpenMU SeasonSix (map chưa có section 1):

```bash
python3 ./scripts/spawn/append-move-map-spawns-from-openmu.py
```
...
```bash
./scripts/spawn/sync-monster-spawn-stack.sh   # enable + sync-all-spawns-from-openmu + import Postgres
```
...
- [`M8-MOVE-MAP-PARITY-CHECKLIST.md`](./world/M8-MOVE-MAP-PARITY-CHECKLIST.md) · P4.3 Monster scope  
- [`M9-NPC-MONSTER-CHECKLIST.md`](./combat/M9-NPC-MONSTER-CHECKLIST.md)  
- [`M8-M10-WORLD-RUNTIME-CHECKLIST.md`](./world/M8-M10-WORLD-RUNTIME-CHECKLIST.md)
