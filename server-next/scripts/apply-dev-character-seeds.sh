...
apply() {
  local f="$1"
  echo "[dev-seed] $f"
  "$ROOT/scripts/apply-sql.sh" "$URI" "$f"
}
...
if command -v dotnet >/dev/null 2>&1 && [[ -f "$ROOT/scripts/migrate-inventory-json-to-db.sh" ]]; then
  echo "[dev-seed] inventory JSON → DB (test.json)"
  TAKUMI_PG_CONNECTION_STRING="$URI" "$ROOT/scripts/migrate-inventory-json-to-db.sh" || true
fi
