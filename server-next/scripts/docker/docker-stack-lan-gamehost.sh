...
if [[ "$SKIP_SQL" -eq 0 ]]; then
  ...
  if [[ -x "$SCRIPTS_ROOT/db/apply-sql.sh" ]]; then
    "$SCRIPTS_ROOT/db/apply-sql.sh" "postgresql://takumi:takumi@127.0.0.1:${PG_PORT}/takumi_runtime" || {
...
echo ""
if [[ -x "$SCRIPTS_ROOT/smoke/smoke-m8.sh" ]]; then
  echo "== M8 smoke (full) =="
  "$SCRIPTS_ROOT/smoke/smoke-m8.sh" --no-recreate || true
  echo ""
elif [[ -x "$SCRIPTS_ROOT/smoke/smoke-m8-move-catalog.sh" ]]; then
  echo "== M8 smoke: MoveMapCatalog =="
  "$SCRIPTS_ROOT/smoke/smoke-m8-move-catalog.sh" || true
  echo ""
fi

echo "== Port listeners (Mac) =="
"$SCRIPTS_ROOT/smoke/check-lan-connect-ports.sh"

echo ""
echo "== Smoke connect (127.0.0.1:${CONNECT_PORT}) =="
if "$SCRIPTS_ROOT/smoke/smoke-connect-from-host.sh" 127.0.0.1 "${CONNECT_PORT}"; then
...
if [[ -n "${LAN_HOST}" ]]; then
  ...
  if "$SCRIPTS_ROOT/smoke/smoke-connect-from-host.sh" "${LAN_HOST}" "${CONNECT_PORT}"; then
