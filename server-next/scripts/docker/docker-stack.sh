...
if [[ -x "$SCRIPTS_ROOT/db/apply-sql.sh" ]]; then
  ...
  "$SCRIPTS_ROOT/db/apply-sql.sh" "$PG_URI" sql/patches/016_character_skill.sql || true
  ...
  if [[ "${TAKUMI_APPLY_DEV_SEEDS:-0}" == "1" || "${TAKUMI_APPLY_DEV_SEEDS:-}" == "true" ]]; then
    ...
    if [[ -x "$SCRIPTS_ROOT/db/apply-dev-character-seeds.sh" ]]; then
      "$SCRIPTS_ROOT/db/apply-dev-character-seeds.sh" "$PG_URI" || true
    else
      "$SCRIPTS_ROOT/db/apply-sql.sh" "$PG_URI" sql/patches/017_seed_mg001_character_skill.sql || true
    fi
  else
